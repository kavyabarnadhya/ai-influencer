...
## 2026-08-15 - Uncompressed vs Compressed NumPy Archives for Cache Files
**Learning:** Using `np.savez_compressed` (DEFLATE compression) on high-frequency, dense numerical `float32` arrays (such as depth maps) is highly CPU-bound and slow. It provides a negligible compression ratio (~11% smaller) on non-redundant float data while introducing massive CPU overhead. Replacing it with uncompressed `np.savez` delivers a ~15.3x write speedup and a ~6.3x read speedup (saving ~500ms on every cache write and ~50ms on every cache load) while remaining 100% backward-compatible with `np.load` for existing files.
**Action:** Avoid `np.savez_compressed` for caching dense numerical arrays (like float32 depths, features, or embeddings). Instead, use `np.savez` (uncompressed archive format) or raw `np.save` to optimize disk I/O and eliminate CPU compression latency.

## 2026-08-20 - LRU Caching for Model Path Resolution in hand_qc
**Learning:** In `scripts/hand_qc.py`, `_find_hand_model()` was re-opening and parsing `config.yaml` on every lookup. Decorating `_find_hand_model` with `@functools.lru_cache(maxsize=1)` delivers a ~660x speedup (down to <0.002ms per call) on warm hits. Additionally, pre-resolving real paths once during batch directory scans avoids redundant `os.path.realpath` syscalls during YOLO inference result mapping.
**Action:** Ensure all model path resolution helpers that read `config.yaml` are decorated with `@functools.lru_cache(maxsize=1)`, and pre-compute real paths once when mapping batch inference outputs.

## 2026-08-25 - Localized Subgrid Distance Math for Unshifted DFT Masks
**Learning:** Evaluating distance equations across full $H \times W$ meshgrids (e.g. `np.ogrid[:h, :w]`) to generate unshifted DFT low-frequency masks evaluates millions of pixels unnecessarily because low-frequency components only reside within $r_{inner}$ of the 4 corners. Calculating distances on $r_{inner} \times r_{inner}$ subgrids and slicing directly into the mask corners achieves a ~238x speedup (~0.67ms vs ~160ms for 1080p) on cold mask creation while maintaining 100% bitwise output equality.
**Action:** In frequency-domain masking, avoid full-frame grid distance evaluations when target regions are spatially localized (e.g., corners or centers). Construct localized subgrids ($r \times r$) and slice the resulting submasks directly into place.
